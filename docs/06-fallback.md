...
**適用對象**：環境問題無法在現場修復，或某一關卡太久跟不上進度的人。
...
第一種在現場就能解決，不用往下讀。
分不清楚的話，回 [Lab 0 的環境檢查](00-lab0-env-check.md)跑一次 preflight——
六項都過就是第一種。
...
存放於 [docs/assets/](assets/)，涵蓋 Lab 3 與 Lab 4 兩段**你沒有親自跑過**的流程。

| 檔名                           | 對應步驟                                                             | 你該從中確認什麼                                  |
| ------------------------------ | -------------------------------------------------------------------- | ------------------------------------------------- |
| `lab3-01-azd-deploy.png`       | [Lab 3 步驟四](03-lab3-deploy.md#步驟四部署然後去休息)               | `azd deploy` 開始建置時的輸出長什麼樣             |
| `lab3-02-agent-active.png`     | [Lab 3 步驟五](03-lab3-deploy.md#步驟五輪詢到-active)                | `azd ai agent show` 顯示 `active` 的樣子          |
| `lab3-03-playground.png`       | [Lab 3 步驟六](03-lab3-deploy.md#步驟六在-playground-送-12-題)       | playground 是與**整個系統**對話，不是挑單一 agent |
...
> ℹ️ 截圖由講師在現場示範時擷取。若 `docs/assets/` 目前是空的，
> 表示這一場的示範尚未進行——向講師索取即可，其餘兩項材料不受影響。
...
照 [Lab 1 講義](01-lab1-single-agent.md)在 portal 建立 `qvn-coding-agent`。
...
依 [Lab 4 講義](04-lab4-publish-teams.md)在 portal 發布。

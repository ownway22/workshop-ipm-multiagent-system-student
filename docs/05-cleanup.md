...
**時機**：Lab 4 完成後，**離開前一定要做**

---

## 為什麼一定要做

課程建立的資源會**持續計費**。agent 本身不貴，但 Lab 3 的 Container Registry
與 Lab 4 的 Bot Service 是按月算的，放著不管就一直扣。

清理只要十分鐘，比下個月收到帳單再回頭找容易得多。
...
這一項是 portal 發布流程自動建立的，`azd down` 刪不到。
...
課程示範的是「怎麼把它跑起來」。真的要放到工作環境用，下面四件事必須先想清楚。
...
| 乾跑把**你自己的** agent 列進待刪清單 | 那個 agent 名稱剛好以 `qvn-` 開頭          | **不要**執行 `--confirm`；改到 portal 手動逐一刪除         |
| 刪除時回權限錯誤                      | 缺專案範圍的 `Foundry Project Manager`     | 請管理員指派後重試，或到 portal 手動刪除                   |
| `azd down` 找不到環境                 | 不在 `deploy/` 目錄，或 azd 環境已被刪     | `cd deploy`；環境不在就到 portal 手動刪 Container Registry |
...
[如果你卡住了](06-fallback.md#會後重跑完整步驟)有完整的重跑步驟，
